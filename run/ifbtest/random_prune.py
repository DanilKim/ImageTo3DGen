...
def info_hack_indices(self):
    with torch.autograd.profiler.record_function(self._profile_name):
        ...
        if isinstance(self._dataset, RandomPrune):
...
class RandomPrune(Dataset):
    ...
    def __init__(self, dataset: Dataset, num_epochs: int,
                 prune_ratio: float = 0.5, delta: float = 0.875, rescale=True):
    ...
    def prune(self):
        ...
        selected_indices = np.random.choice(np.arange(len(self.dataset)), int(
            self.keep_ratio * len(self.dataset)), replace=False)
        self.reset_weights()
        if len(selected_indices) > 0:
            if self.rescale:
                self.weights[selected_indices] = 1 / self.keep_ratio
        self.num_pruned_samples += (1-self.keep_ratio) * len(self.dataset)
...
class IBSampler(object):
    def __init__(self, dataset: RandomPrune):
        self.dataset = dataset
