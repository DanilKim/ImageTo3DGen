...
if __name__ == '__main__':
    ...
    datatype = opt.data_dir.split('/')[-1]
    opt.workspace = os.path.basename(args.config).replace('.yaml', '')
    opt.workspace = os.path.join(
        'log', 
        os.path.basename(args.config).split('.')[0],
        f'single_data_bs{opt.batch_size}_nr{opt.num_rays_per_image}_ns{opt.num_steps}_lr{opt.lr}_aabb{opt.bound}_mask'
        #f'{datatype}_bs{opt.batch_size}_nr{opt.num_rays_per_image}_ns{opt.num_steps}_ld{opt.lambda_dist:.1f}_lsm{opt.lambda_smooth:.1f}_lsp{opt.lambda_sparse:.1f}'
    ...
    if opt.test:
        ...
        test_loader = MultiviewImages(opt, data_dir=opt.data_dir, device=device, training=False).dataloader()
        
        num_objects = len(test_loader._data.subjects)
        ...
        trainer = Trainer('mlp_decoder', opt, model, device=device, workspace=opt.workspace, fp16=opt.fp16, use_checkpoint=opt.ckpt)
        ...
        if opt.save_mesh:
            trainer.save_mesh(resolution=64)
            
    else:
        
        train_loader = MultiviewImages(opt, data_dir=opt.data_dir, device=device, training=True).dataloader()
        
        num_objects = len(train_loader._data.subjects)
        ...
        scheduler = optim.lr_scheduler.LambdaLR(optimizer, lambda epoch: 1) #0.95**epoch) # fixed
