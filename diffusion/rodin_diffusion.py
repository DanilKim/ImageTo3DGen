...
class DiffusionUpsampler(nn.Module):
    def __init__(self, opt, device):
        super().__init__()

        
class LatentDissufion(nn.Module):
    def __init__(self, opt, device):
        super().__init__()
        
# From NeuralLift-360 (https://github.com/VITA-Group/NeuralLift-360/blob/main/nerf/sd.py)
class TextCondDiffusion(nn.Module):
    def __init__(self, opt, device, clip_name='laion/CLIP-ViT-B-32-laion2B-s34B-b79K'):
        super().__init__()
        ...
        self.tokenizer = CLIPTokenizer.from_pretrained(clip_name, subfolder="tokenizer")
        # self.tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-large-patch14")
        self.text_encoder = CLIPTextModel.from_pretrained(clip_name, subfolder="text_encoder").to(self.device)
        # self.text_encoder = CLIPTextModel.from_pretrained("openai/clip-vit-large-patch14").to(self.device)

        print(f'[INFO] Text-cond diffusion!')
