...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, model, x_t:torch.Tensor, t:torch.Tensor, **model_kwargs):
        """
        ...
        pred_eps_cond = model(x_t, t, **model_kwargs)
        model_kwargs['cemb'] = torch.zeros(cemb_shape, device = self.device)
        pred_eps_uncond = model(x_t, t, **model_kwargs)
