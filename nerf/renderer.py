...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        # camera
        self.bound = opt.bound # assume the scene is bounded in box(-bound, bound)
        self.cascade = 1 + math.ceil(math.log2(self.bound))
        self.grid_size = 128
        self.min_near = opt.min_near # minimum near distance for camera
        
        # ngp
        self.cuda_ray = False # use CUDA raymarching instead of pytorch, not supported
        self.density_thresh = opt.density_thresh # threshold for density grid to be occupied
        self.bg_radius = opt.bg_radius # if positive, use a background model at sphere(bg_radius)
        
        # resisual blob
        self.blob_density = opt.blob_density # max (center) density for the density blob
        self.blob_radius = opt.blob_radius # control the radius for the density blob
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, perturb=False, **kwargs):
        ...
        sigmas, rgbs, normals = self(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), l=light_d, l_a=kwargs.get('l_a'), l_p=kwargs.get('l_p'), ratio=ambient_ratio, shading=shading)
